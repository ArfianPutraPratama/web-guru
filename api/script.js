...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      // Browser mengirim 'text/plain', jadi 'request.body' di Vercel adalah Teks mentah.
      // Kita tidak boleh JSON.stringify() lagi. Kita harus meneruskan teks mentah apa adanya.
      
      googleResponse = await fetch(GOOGLE_SCRIPT_URL, {
        method: "POST",
        redirect: "follow",
        headers: { "Content-Type": "text/plain;charset=utf-8" },
        body: request.body, // <-- HAPUS "JSON.stringify()" DARI SINI
      });
      // --- BATAS PERBAIKAN ---
